...
class plane : public hitable
{
    public:
        plane();
        explicit plane(float constant);
        plane(const vec3<float>& coefficients, float constant);
        plane(const vec3<float>& center, const vec3<float>& coefficients, float constant);
